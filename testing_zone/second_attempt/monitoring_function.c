...
/*   Updated: 2024/03/18 12:28:17 by akuburas         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	monitoring(t_pointers *data, int *argv_int)
{
	...
	set_to_dead = 0;
	i = 0;
	printf("Inside monitoring.\n");
	while (1)
	{
		printf("Inside monitoring loop.\n");
		if (i == argv_int[0])
			i = 0;
		printf("Monitoring loop trying to lock monitor %d\n", i);
		printf("This is the address of the monitor mutex inside monitoring = %p\n", &data->monitors[i]);
